...
class BasicPowerSGD(Aggregator):
    def __init__(self, params: List[torch.Tensor], config: BasicConfig):
        ...
        # _init_p_batch and _init_q_batch was here!!
    ...
    def aggregate(self, gradients: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        # initialize the p and q based on the distribution of the gradients
        for i in range(len(gradients)):
            print(f'standard deviation gradients[{i}].std() = ', gradients[i].std())
        print('self.step_counter = ', self.step_counter)
        if self.step_counter == 0:
            self._initializze_p_and_q_batch(gradients_per_shape)
        ...
        for it in range(num_iters_per_step):
            ...
            if iter_is_even:
                ...
                print(f'estimating Q {sum([torch.numel(part_of_q) for part_of_q in self._qs])}')
            else:
                maybe_transpose = batch_transpose
                out_batches, in_batches = self._ps, self._qs
                out_buffer = self._ps_buffer
                print(f'estimating P {sum([torch.numel(part_of_p) for part_of_p in self._ps])}')
            ...
            # np.savetxt('in_batches_before.txt', in_batches[0].numpy().reshape(-1))
            # np.savetxt('out_batches_before.txt', out_batches[0].numpy().reshape(-1))
            ...
            # np.savetxt('in_batches_after.txt', in_batches[0].numpy().reshape(-1))
            # np.savetxt('out_batches_after.txt', out_batches[0].numpy().reshape(-1))

        for group, ps, qs in zip(
                shape_groups, self._ps, self._qs
        ):
            group["grad_batch"].baddbmm_(
                ps,
                batch_transpose(qs), 
                alpha=-1
            )
    ...
    def aggregate_using_rescaled_grads(self, gradients: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        for it in range(num_iters_per_step):
            print('Hi')
            # Alternate between left and right matrix multiplications
            iter_is_even = (self.step_counter_rescaled * num_iters_per_step + it) % 2 == 0
            if iter_is_even:
                maybe_transpose = lambda g: g
        ...
        for grad_batch, ps_rescaled, qs_rescaled in zip(grad_batches, self._ps_rescaled, self._qs_rescaled):
            grad_batch.baddbmm_(
                ps_rescaled, 
                batch_transpose(qs_rescaled),
                alpha=-1
            )

        for approx, ps_rescaled, qs_rescaled in zip(approximation, self._ps_rescaled, self._qs_rescaled):
            approx.baddbmm_(
                ps_rescaled, 
                batch_transpose(qs_rescaled),
            )
        ...
        for i, (num_elements, shape) in enumerate(unsqueezed_shapes):
            ...
            gradients[i].copy_(single_dim_error[left:right].view(shape))
    ...
    def _initializze_p_and_q_batch(self, gradients_per_shape):
        p_to_pack, q_to_pack = [], []
        
        # for shape, grads in gradients_per_shape.items():
        #     rank = min(self.config.rank, min(shape))
        #     p_to_pack.append(torch.randn([len(grads), shape[0], rank], generator=self.generator))
        
        # self._ps_buffer, ps_shapes = pack(p_to_pack)
        # self._ps = unpack(self._ps_buffer, ps_shapes)
        
        # for shape, grads in gradients_per_shape.items():
        #     rank = min(self.config.rank, min(shape))
        #     q_to_pack.append(torch.randn([len(grads), shape[1], rank], generator=self.generator))

        # self._qs_buffer, qs_shapes = pack(q_to_pack)
        # self._qs = unpack(self._qs_buffer, qs_shapes)            
            
        for shape, grads in gradients_per_shape.items():

            p_to_stack, q_to_stack = [], []
            rank = min(self.config.rank, min(shape))
            
            for g in grads:
                p_to_stack.append(torch.normal(mean=0, std=1, size=(g.shape[0], rank)))
            
            for g in grads:
                q_to_stack.append(torch.normal(mean=0, std=1, size=(g.shape[1], rank)))
                
            p_to_pack.append(torch.stack(p_to_stack))
            q_to_pack.append(torch.stack(q_to_stack))
